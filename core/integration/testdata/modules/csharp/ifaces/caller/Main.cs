...
public class Caller
{
    ...
    public async Task Test()
    {
        ...
        await test.Void(impl.AsInterface<TestCustomIface>());

        // String tests
        var str = await test.Str(impl.AsInterface<TestCustomIface>());
        if (str != "a")
            throw new Exception($"Expected 'a', got '{str}'");

        var withStrResult = await test.WithStr(impl.AsInterface<TestCustomIface>(), "c").Str();
        if (withStrResult != "c")
            throw new Exception($"Expected 'c', got '{withStrResult}'");

        // Optional string tests
        var withOptionalStr1 = await test.WithOptionalStr(impl.AsInterface<TestCustomIface>(), "d").Str();
        if (withOptionalStr1 != "d")
            throw new Exception($"Expected 'd', got '{withOptionalStr1}'");

        var withOptionalStr2 = await test.WithOptionalStr(impl.AsInterface<TestCustomIface>()).Str();
        if (withOptionalStr2 != "a")
            throw new Exception($"Expected 'a', got '{withOptionalStr2}'");

        // String list tests
        var strList = await test.StrList(impl.AsInterface<TestCustomIface>());
        ...
        // Int tests
        var intVal = await test.Int(impl.AsInterface<TestCustomIface>());
        if (intVal != 1)
            throw new Exception($"Expected 1, got {intVal}");

        var withIntResult = await test.WithInt(impl.AsInterface<TestCustomIface>(), 3).IntValue();
        if (withIntResult != 3)
            throw new Exception($"Expected 3, got {withIntResult}");

        // Bool tests
        var boolVal = await test.Bool(impl.AsInterface<TestCustomIface>());
        if (!boolVal)
            throw new Exception($"Expected true, got {boolVal}");

        // Self interface tests
        var selfIface = test.SelfIface(impl.AsInterface<TestCustomIface>());
        var selfStr = await selfIface.Str();
        if (selfStr != "aself")
            throw new Exception($"Expected 'aself', got '{selfStr}'");

        var selfIfaceList = await test.SelfIfaceList(impl.AsInterface<TestCustomIface>());
        if (selfIfaceList.Count() != 2)
            throw new Exception($"Expected 2 self ifaces, got {selfIfaceList.Count()}");

        // Other interface tests
        var otherIface = test.OtherIface(impl.AsInterface<TestCustomIface>());
        var otherStr = await otherIface.Foo();
        if (otherStr != "aother")
            throw new Exception($"Expected 'aother', got '{otherStr}'");

        var staticOtherList = await test.StaticOtherIfaceList(impl.AsInterface<TestCustomIface>());
        ...
        var dynamicEmpty = await test.DynamicOtherIfaceList(impl.AsInterface<TestCustomIface>());
        ...
        var withOther1 = test.WithOtherIface(
            impl.AsInterface<TestCustomIface>(),
        ...
        var ifaceListArgs = await test.IfaceListArgs(
            [impl.AsInterface<TestCustomIface>(), impl.SelfIface().AsInterface<TestCustomIface>()],
        ...
        var parentFields = await test.WithIface(impl.AsInterface<TestCustomIface>())
            .WithPrivateIface(privateImpl.AsInterface<TestCustomIface>())
            .WithIfaceList([impl.AsInterface<TestCustomIface>(), impl.SelfIface().AsInterface<TestCustomIface>()])
            .WithOtherIfaceList([
        ...
        var withOptional1 = await test.WithOptionalIface()
            .WithOptionalIface(impl.AsInterface<TestCustomIface>())
            .WithOptionalIface()
            .ParentIfaceFields();
