...
{
    [Function]
    public async Task<string> GetUser(string gender)
    {
...
{
    [Function]
    public async Task<string> GetUser(bool french)
    {
        var uri = french 
            ? "https://randomuser.me/api/?nat=fr" 
            : "https://randomuser.me/api/";
            
        return await Dag.Container()
            .From("alpine:latest")
            .WithExec(["apk", "add", "curl", "jq"])
            .WithExec(["sh", "-c", $"curl {uri} | jq .results[0].name"])
            .Stdout();
    }
}
```
...
{
    [Function]
    public async Task<string> Tree(Directory src, string depth)
    {
        return await Dag.Container()
            .From("alpine:latest")
...
{
    [Function]
    public async Task<string> Grep(string pattern, File source)
    {
        return await Dag.Container()
            .From("alpine:latest")
            .WithMountedFile("/src", source)
            .WithExec(["grep", pattern, "/src"])
            .StdoutAsync();
    }
}
```
...
{
    [Function]
    public async Task<string> Scan(Container ctr)
    {
