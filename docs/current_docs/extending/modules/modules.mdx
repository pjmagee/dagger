...
:::note
By default, the description will be read from the XML documentation of the class or function. To define a different
...
The Dagger module's code in C# can be split into multiple files. A C# module has the following structure:
...
You can create additional C# files alongside `Main.cs`:
...
{
    [Function]
    public async Task<string> Run(Directory source)
    {
...
:::note
When you run `dagger functions`, you'll only see the functions from the main `MyModule` class (`test` and `lint`). The functions from the `Test` and `Lint` classes are accessible by chaining, for example: `dagger call test run --source=.` or `dagger call lint run --source=.`.
:::
...
<TabItem value="csharp" label="C#">
Dagger modules in C# are built as DLLs using the .NET CLI. The runtime container builds the module code with:
...
The generated DLL is then executed by the runtime container.

:::important
This means you have to keep the `.csproj` file and SDK configurations.
:::
</TabItem>
</Tabs>
