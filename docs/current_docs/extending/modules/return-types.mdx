...
{
    [Function]
    public async Task<string> OsInfo(Container ctr)
    {
        return await ctr.WithExec(["uname", "-a"]).StdoutAsync();
    }
}
```
</TabItem>
...
{
    [Function]
    public Container AlpineBuilder(string[] packages)
    {
        var ctr = Dag.Container().From("alpine:latest");
