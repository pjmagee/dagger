...
{
    [Function]
    public async Task<string> Scan(string @ref, Severity severity)
    {
        return await Dag.Container()
            .From(@ref)
            .WithExec(["apk", "add", "curl"])
            .WithExec(["sh", "-c", $"echo Severity: {severity}"])
            .StdoutAsync();
    }
}
```
...
:::note
C# enums are automatically recognized by Dagger. Enum values should follow PascalCase convention and will be converted to uppercase in the API.
:::
</TabItem>
