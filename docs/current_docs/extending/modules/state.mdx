...
:::warning
Dagger only exposes public properties; private fields will not be exposed unless explicitly marked with `[Function]`.
:::

Here's an example where one property is exposed as a Dagger Function, while the other is not:
