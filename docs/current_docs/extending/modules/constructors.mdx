...
public class MyModule
{
    ...
    public MyModule(string greeting = "Hello", string name = "World")
    {
        Greeting = greeting;
        Name = name;
    }
...
:::info
In the C# SDK, public properties are automatically serialized. Use required or constructor parameters to ensure values are provided.
:::
...
public class MyModule
{
    ...
    public MyModule(Container? ctr = null)
    {
        Ctr = ctr ?? Dag.Container().From("alpine:latest");
    }

    [Function]
    public async Task<string> Foo()
    {
        return await Ctr.WithExec(["echo", "foo"]).StdoutAsync();
    }

    [Function]
    public async Task<string> Bar()
    {
        return await Ctr.WithExec(["echo", "bar"]).StdoutAsync();
    }
}
```
