...
public class MyModule
{
    ...
    public async Task<string> Test()
    {
        return await Dag.Container()
            .From("alpine")
            .WithExec(["sh", "-c", "echo hello world; exit 1"])
            .StdoutAsync();
    }
}
```
...
{
    [Function]
    public async Task<string> Build(Directory src)
    {
        ...
        await builder.Terminal();

        var prod = Dag.Container()
            .From("alpine:latest")
            .WithFile("/myapp", builder.File("/src/myapp"))
            .WithEntrypoint(["/myapp"]);

        await prod.Terminal();

        return await prod.Publish("ttl.sh/myapp");
    }
}
```
</TabItem>
