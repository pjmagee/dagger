...
public class MyModule
{
    ...
    public Service HttpService()
    {
        return Dag.Container()
            .From("python:alpine")
            .WithWorkdir("/srv")
            .WithNewFile("index.html", "Hello from Dagger!")
            .WithExposedPort(8080)
            .AsService(
                new ContainerAsServiceOpts { Args = ["python", "-m", "http.server", "8080"] }
            );
    }
}
```
...
{
    [Function]
    public async Task<string> UserList(Service mariadb)
    {
        // expose host service to Dagger
        var mariadbSrv = Dag.Host().Service([new PortForward { Frontend = 3306, Backend = 3306 }]);

        return await Dag.Container()
            .From("mariadb:latest")
            .WithServiceBinding("db", mariadbSrv)
            .WithExec(["mariadb", "--user=root", "--password=secret", "--host=db", "-e", "SELECT Host, User FROM mysql.user"])
            .StdoutAsync();
    }
}
```
