...
Here's an example of a Dagger Function that excludes everything in a given directory except C# source code files:

```cs
using Dagger;
using static Dag = Dagger.Client;

[Object]
public class MyModule
{
    [Function]
    public async Task<Container> Foo([Ignore("*", "!**/*.cs", "!*.csproj")] Directory source)
    {
...
<TabItem value="csharp" label="C#">
```cs
// exclude C# tests and test data
[Ignore("**/*Tests.cs", "**/TestData/**")]
...
To implement a post-call filter in your Dagger Function, use the `ContainerWithDirectoryOpts` class which supports `Include` and `Exclude` properties when working with `Directory` objects. Here's an example:
...
public class MyModule
{
    ...
    {
        var builder = Dag.Container()
            .From("golang:latest")
            .WithDirectory("/src", source, new ContainerWithDirectoryOpts
            {
        ...
        return Dag.Container()
            .From("alpine:latest")
            .WithDirectory("/app", builder.Directory("/src/hello"), new ContainerWithDirectoryOpts
            {
                Include = ["hello.bin"]
            })
...
var dirOpts = new ContainerWithDirectoryOpts
{
    Exclude = ["*.md*"]
};
...
{
    [Function]
    public Directory Debug(
        [Ignore("*", "!analytics")] Directory source)
    {
        return source;
    }
}
