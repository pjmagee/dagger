...
This quickstart will take you approximately 10 minutes to complete. You should be familiar with programming in C#, Go, Python, TypeScript, PHP, or Java.
...
To use this function, initialize a new Dagger module using the command below, and then update the auto-generated `Main.cs` file to include the function code above:

```shell
dagger init --sdk=csharp --name=basics
```
</TabItem>
...
{
    return Base().WithExec(["apk", "add", "bash", "git"]);
}
[Function]
public async Task<string> BuildAndPublish()
{
    return await Build().Publish("ttl.sh/bar");
}
```
...
{
    [Function]
    public Container Build(string image = "alpine:latest")
    {
        return Dag.Container()
            .From(image)
            .WithNewFile("/hi.txt", "Hello from Dagger!");
    ...
    [Function]
    public async Task<string> Publish(string image = "alpine:latest")
    {
...
public class Basics
{
    ...
    {
        var ctr = Dag.Wolfi().Container();
        return await Dag.Trivy().ScanContainer(ctr);
    }
}
```
</TabItem>
...
public class Basics
{
    ...
    {
        var aptCache = Dag.CacheVolume("apt-cache");
        return Dag.Container()
            .From("debian:latest")
