...
Bootstrap a new Dagger module in C#, Go, Python, TypeScript, PHP, or Java by running `dagger init` in the application's root directory.
...
```shell
dagger init --sdk=csharp --name=hello-dagger
```
...
Replace the generated `Main.cs` file with the following code, which adds four Dagger Functions to your Dagger module:
...
{
    [Function]
    public async Task<string> Publish(Directory source)
    {
        await Test(source);
        return await Build(source)
            .Publish($"ttl.sh/hello-dagger-{Guid.NewGuid():N}");
    }

    [Function]
    public async Task<string> Test(Directory source)
    {
        return await BuildEnv(source)
