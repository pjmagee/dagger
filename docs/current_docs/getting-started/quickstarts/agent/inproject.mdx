...
<Tabs groupId="language" queryString="sdk">
    ...
    ```shell
    dagger init --sdk=csharp .dagger/workspace
    ```

    Replace the `.dagger/workspace/Main.cs` file with the following contents:
    ```cs
using Dagger;
...
{
    public required Directory Source { get; set; }
    [DaggerFunction]
    public async Task<string> ReadFile(string path)
    {
        return await Source.File(path).ContentsAsync();
    }
    [DaggerFunction]
    public Workspace WriteFile(string path, string contents)
    {
        Source = Source.WithNewFile(path, contents);
        return this;
    }
    [DaggerFunction]
    ...
    {
        var nodeCache = Dag.CacheVolume("node");
        return await Dag.Container()
            .From("node:21-slim")
            .WithDirectory("/src", Source)
...
<Tabs groupId="language" queryString="sdk">
    <TabItem value="csharp" label="C#">
        Add the following new function to the main Dagger module in `.dagger/Main.cs`:

        ```cs
[Function]
public async Task<Directory> Develop(string assignment, [DefaultPath("/")] Directory source)
{
    ...
    var work = Dag.Llm().WithEnv(environment).WithPromptFile(promptFile);

    // Get the output from the agent
    var completed = work.Env().Output("completed").AsWorkspace();
    var completedDirectory = completed.Source.WithoutDirectory("node_modules");

    // Make sure the tests really pass
    await Test(completedDirectory);
...
<Tabs groupId="language" queryString="sdk">
    ...
    ```cs
[Function]
public async Task<string> DevelopIssue(
    Secret githubToken,
    int issueId,
    string repository,
    [DefaultPath("/")] Directory source)
{
    ...
    var issueClient = Dag.GithubIssue(new GithubIssueOpts { Token = githubToken });
    var issue = await issueClient.Read(repository, issueId);

    // Get information from the Github issue
    var assignment = await issue.Body();

    // Solve the issue with the Develop agent
    var feature = await Develop(assignment, source);

    // Open a pull request
    var title = await issue.Title();
    var url = await issue.Url();
    var body = $"{assignment}\n\nCloses {url}";
    var pr = await issueClient.CreatePullRequest(repository, title, body, feature, "main");
