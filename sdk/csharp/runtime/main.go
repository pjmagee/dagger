...
func (m *CsharpSdk) CodegenBase(
	...
	analyzerBuild := base.
		...
		WithWorkdir("/analyzer-src/src/Dagger.SDK.Analyzers").
	...
	analyzerDll := analyzerBuild.File("/analyzer-src/src/Dagger.SDK.Analyzers/bin/Release/netstandard2.0/Dagger.SDK.Analyzers.dll")

	// Build the code fixes DLL separately
	codeFixesBuild := base.
		WithDirectory("/codefixes-src", m.SourceDir).
		WithWorkdir("/codefixes-src/src/Dagger.SDK.CodeFixes").
	...
	codeFixesDll := codeFixesBuild.File("/codefixes-src/src/Dagger.SDK.CodeFixes/bin/Release/netstandard2.0/Dagger.SDK.CodeFixes.dll")
	...
	sdkSource := base.
		...
		WithWorkdir("/sdk-src/src/Dagger.SDK").
		...
		Directory("/sdk-src/src/Dagger.SDK")
	...
	for _, part := range parts {
		if len(part) > 0 {
			...
			if firstRune >= 'a' && firstRune <= 'z' {
				firstRune -= 32 // Convert to uppercase
			}
			rest := ""
			if len(part) > 1 {
				rest = part[1:]
