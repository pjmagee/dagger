...
public class ClientIntegrationTests
{
    ...
    public async Task TestContainerWithMultipleExec()
    {
        var output = await Dag
            .Container()
            .From("alpine:latest")
            .WithExec(new[] { "sh", "-c", "echo foo && echo bar" })
            .StdoutAsync();
    ...
    public async Task TestContainerEnvironmentVariables()
    {
        var output = await Dag
            ...
            .WithExec(new[] { "sh", "-c", "echo $TEST_VAR" })
            .StdoutAsync();
    ...
    public async Task TestDirectoryOperations()
    {
        var dir = Dag
            .Directory()
            .WithNewFile("hello.txt", "Hello from Dagger SDK!");

        var fileContents = await Dag
            .Container()
            .From("alpine:latest")
    ...
    {
        var dockerfile = @"FROM alpine:latest
...
";
    ...
    public async Task TestChainingOperations()
    {
        var result = await Dag
            ...
            .WithExec(new[] { "sh", "-c", "echo $GREETING $NAME" })
            .StdoutAsync();
    ...
    public async Task TestCacheVolume()
    {
        ...
        await Dag
            ...
            .WithExec(new[] { "sh", "-c", "echo 'cached data' > /cache/data.txt" })
            .SyncAsync();
        ...
        var output = await Dag
            ...
            .From("alpine:latest")
            .WithMountedCache("/cache", Dag.CacheVolume("test-cache"))
            .WithExec(new[] { "cat", "/cache/data.txt" })
            .StdoutAsync();
