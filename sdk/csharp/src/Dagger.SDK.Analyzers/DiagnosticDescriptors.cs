...
public static class DiagnosticDescriptors
{
    ...
    public static readonly DiagnosticDescriptor DefaultPathAttributeOnInvalidParameterType = new(
        id: "DAGGER010",
        title: "[DefaultPath] attribute can only be applied to Directory, File, GitRepository or GitRef parameters",
        messageFormat: "Parameter '{0}' has [DefaultPath] attribute but is of type '{1}'. [DefaultPath] can only be used on Directory, File, GitRepository or GitRef parameters",
        category: Category,
        defaultSeverity: DiagnosticSeverity.Error,
        isEnabledByDefault: true,
        description: "The [DefaultPath] attribute is only valid on parameters whose type supports contextual resolution: Directory, File, GitRepository or GitRef (the engine resolves contextual git arguments from the module's repository). Other parameter types are rejected at runtime."
    );
