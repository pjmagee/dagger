...
public static partial class Entrypoint
{
    private static readonly JsonSerializerOptions SerializerOptions = new()
    {
        ...
        TypeInfoResolver = new System.Text.Json.Serialization.Metadata.DefaultJsonTypeInfoResolver
        {
            Modifiers =
            {
                static typeInfo =>
                {
                    ...
                    // Properties renamed via [Function(Name = "...")] must
                    // serialize under the registered field name so the
                    // engine's state lookup by OriginalName succeeds.
                    foreach (var prop in typeInfo.Properties)
                    {
                        if (prop.AttributeProvider is not MemberInfo member)
                        {
                            continue;
                        }

                        var fnAttr = member.GetCustomAttribute<FunctionAttribute>();
                        if (!string.IsNullOrWhiteSpace(fnAttr?.Name))
                        {
                            prop.Name = ToCamelCase(fnAttr!.Name!);
                        }
                    }
                },
            },
        ...
        Converters =
        {
            ...
            // Dagger objects and interface values held in module state
            // round-trip as id strings.
            new DaggerValueConverterFactory(),
        },
    };
    ...
    private static async Task<int> RunInternalAsync(string[] args)
    {
        ...
        try
        {
            (moduleInfos, interfaceInfos) = BuildModuleTypeInfos();
        }
        catch (Exception ex)
        {
    ...
    )
    {
        ...
        try
        {
            ...
            foreach (var moduleInfo in moduleInfos)
            {
                ...
                // Register the constructor even when it takes no arguments:
                // without a registered constructor the engine synthesizes the
                // object with EMPTY state and never invokes the module, so
                // property initializers would not serialize and every field
                // would read as its zero value.
                ...
                foreach (var field in moduleInfo.Fields)
                {
                    ...
                    try
                    {
                        (fieldTypeDef, fieldIsNullable) = BuildTypeDef(
                            dag,
                            field.PropertyType,
                            propertyInfo: field.PropertyInfo
                        );
                    }
                    catch when (!field.Explicit)
                    {
                        // Auto-exposed properties whose type has no Dagger
                        // mapping are silently skipped; explicit [Function]
                        // fields still fail registration hard.
                        continue;
                    }
                    if (fieldIsNullable)
                    {
    ...
    )
    {
        ...
        try
        {
            ...
            if (moduleInfo.Fields.Count > 0)
            {
                ...
                var ctorParamsForSkip =
                    moduleInfo.ConstructorMethod?.GetParameters()
                    ?? moduleInfo.Constructor?.GetParameters();
                if (ctorParamsForSkip != null)
                {
                    foreach (var param in ctorParamsForSkip)
                    {
                ...
                foreach (var fieldInfo in moduleInfo.Fields)
                {
                    ...
                    if (parentElement.TryGetProperty(fieldInfo.Name, out var fieldElement))
                    {
                        ...
                        // Auto-exposed get-only properties are serialized but cannot
                        // be rehydrated; skip them instead of crashing on SetValue.
                        if (fieldInfo.PropertyInfo.CanWrite)
                        {
    ...
    )
    {
        ...
        for (var i = 0; i < constructorParams.Length; i++)
        {
            ...
            // Registration honors [Name] on constructor parameters, so the
            // engine sends the argument under the API name; parent-state keys
            // on reconstruction use serialized property names, hence the
            // CLR-name fallback.
            var apiName = param.GetCustomAttribute<NameAttribute>()?.ApiName;

            JsonElement argElement = default;
            var found =
                (apiName is not null && argumentMap.TryGetValue(apiName, out argElement))
                || argumentMap.TryGetValue(paramName, out argElement);
            ...
            {
                throw new InvalidOperationException(
                    $"Missing required constructor argument '{apiName ?? paramName}'."
                );
            }
        }
    ...
    ) BuildModuleTypeInfos()
    {
        ...
        foreach (var type in types)
        {
            ...
            if (daggerAttr is not null)
            {
                ...
                foreach (var method in type.GetMethods(BindingFlags.Instance | BindingFlags.Public))
                {
                    ...
                    var functionInfo = new FunctionInfo
                    {
                        ...
                        Deprecated = functionAttr.Deprecated
                            ...
                            ?? method.GetCustomAttribute<DeprecatedAttribute>()?.Message,
                ...
                {
                    // Public properties are exposed as fields automatically
                    // (Go/Java parity: exported/public state is API surface);
                    // [Function] on a property is an optional override for
                    // name/description/deprecation.
                    if (property.GetIndexParameters().Length > 0)
                    {
                        continue; // indexers cannot be fields
                    }
                    ...
                    var fieldInfo = new DaggerFieldInfo
                    {
                        ...
                        Description = fieldAttr?.Description ?? GetXmlDocumentation(property),
                        Deprecated = fieldAttr?.Deprecated
                            ?? property.GetCustomAttribute<ObsoleteAttribute>()?.Message
                            ?? property.GetCustomAttribute<DeprecatedAttribute>()?.Message,
                        PropertyInfo = property,
                        PropertyType = property.PropertyType,
                        ApiName = nameAttr?.ApiName,
                        Explicit = fieldAttr is not null,
                    };
            ...
            // Process interface types. Generated bindings (namespace Dagger)
            // also carry [Interface] — e.g. the built-in Node/Exportable/Syncer
            // and interfaces from dependency modules — but those belong to the
            // engine or other modules and must not be registered as this
            // module's own.
            ...
            if (interfaceAttr is not null && type.IsInterface && type.Namespace != "Dagger")
            {
    ...
    )
    {
        ...
        if (typeof(DaggerObject).IsAssignableFrom(targetType))
        {
            ...
            // Objects are reloaded via the Relay-style `node(id:) { ... on Type }`
            // lookup (the legacy per-type `loadTFromID` endpoints no longer exist).
            var nodeQuery = GraphQL.QueryBuilder
                .Builder()
                .Select(
                    "node",
                    System.Collections.Immutable.ImmutableList.Create<GraphQL.Argument>(
                        new GraphQL.Argument("id", new GraphQL.StringValue(id))
                    )
                )
                .Select("... on " + targetType.Name);
            return Activator.CreateInstance(targetType, nodeQuery, dag.GraphQLClient);
        }
    ...
    private static async Task<object?> NormalizeResultAsync(object? value)
    {
        ...
        switch (value)
        {
            ...
            case Scalar scalar:
                return scalar.Value;
            // Dynamic interface proxies (received or converted interface
            // values) normalize to their underlying id.
        ...
        {
            // Emit the same surface as field discovery: all public readable
            // properties, keyed by the registered field name ([Function(Name)]
            // override or camelCased property name).
    ...
    private static async Task ReturnError(Query dag, FunctionCall fnCall, string message)
    {
        ...
        {
            // Never let error reporting mask the root cause: surface both the
            // original failure and the reporting failure on stderr.
            Console.Error.WriteLine($"failed to report error to engine: {reportEx}");
            Console.Error.WriteLine($"original error: {message}");
        }
    }

    /// <summary>
    /// Resolves the engine-side GraphQL type name for a module interface: the
    /// registered local name prefixed with the current module name (e.g.
    /// "Test" + "Duck" -> "TestDuck").
    /// </summary>
    internal static string ResolveInterfaceGraphQLName(Type interfaceType)
    {
        // Generated interfaces (namespace Dagger, e.g. from dependency-module
        // bindings) already carry their engine-side name — use it as-is.
        if (interfaceType.Namespace == "Dagger")
        {
            return interfaceType.GetCustomAttribute<InterfaceAttribute>()?.Name
                ?? interfaceType.Name;
        }

        string localName;
        if (InterfaceTypeRegistry.TryGetValue(interfaceType, out var registered))
        {
            localName = registered;
        }
        else
        {
            var attr = interfaceType.GetCustomAttribute<InterfaceAttribute>();
            localName = attr?.Name ?? interfaceType.Name;
        }

        return string.IsNullOrWhiteSpace(_currentModuleName)
            ? localName
            : _currentModuleName + localName;
    }
    ...
    private static IReadOnlyList<EnumTypeInfo> BuildEnumTypeInfos()
    {
        ...
        {
            // The [Enum] attribute is optional: any public enum in the module
            // assembly is registered (function signatures may reference plain
            // enums, and the engine requires every referenced enum to exist).
            // The attribute only customizes the name/description.
    ...
    private static MethodInfo? GetAlternativeConstructor(Type moduleType)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Type '{moduleType.Name}' has multiple methods marked with [Constructor] ({string.Join(", ", constructorMethods.Select(m => m.Name))}). Only one constructor is allowed per module."
            );
        }
    ...
    private static ConstructorInfo? GetModuleConstructor(Type moduleType)
    {
        ...
        // Prefer a parameterized constructor, but fall back to the default
        // one: the constructor must be registered even when parameterless,
        // because without it the engine synthesizes the object with EMPTY
        // state — the module is never invoked and property initializers never
        // serialize, so every field would read as its zero value.
    ...
    /// <summary>
    /// Gets XML documentation for a property (member id prefix "P:").
    /// </summary>
    private static string? GetXmlDocumentation(PropertyInfo property)
    {
        LoadXmlDocumentation();
        ...
        return ExtractSummary($"P:{property.DeclaringType?.FullName}.{property.Name}");
    }

    private static string MethodMemberId(MethodInfo method)
    {
    ...
    /// <summary>
    /// Resolves a method's XML-doc summary, following the same chain the C#
    /// compiler documents for &lt;inheritdoc/&gt;: the member's own summary,
    /// an explicit cref target, the base-class override chain, then implicitly
    /// implemented interface members.
    /// </summary>
    private static string? GetMethodSummary(MethodInfo method, int depth = 0)
    {
        if (depth > 8)
        {
            return null;
        ...
        var member = FindMemberElement(MethodMemberId(method));
        var summary = SummaryText(member);
        if (summary != null)
        {
            return summary;
        }
        ...
        if (member != null && inherit == null)
        {
            return null; // documented member without summary or inheritdoc
        }

        var cref = inherit?.Attribute("cref")?.Value;
        if (cref != null)
        {
            return ExtractSummary(cref);
        }

        // Base-class override chain.
        var baseDef = method.GetBaseDefinition();
        if (baseDef.DeclaringType != method.DeclaringType)
        {
            var s = GetMethodSummary(baseDef, depth + 1);
            if (s != null)
            {
                return s;
            }
        ...
        // Implicitly implemented interface members.
        ...
        if (declaring != null && !declaring.IsInterface)
        {
            foreach (var iface in declaring.GetInterfaces())
            {
                ...
                {
                    if (map.TargetMethods[i].HasSameMetadataDefinitionAs(method))
                    {
                        var s = GetMethodSummary(map.InterfaceMethods[i], depth + 1);
                        if (s != null)
                        {
                            return s;
                        }
    ...
    private static System.Xml.Linq.XElement? FindMemberElement(string memberName)
    {
    ...
    private static string? SummaryText(System.Xml.Linq.XElement? member)
    {
    ...
    {
        return SummaryText(FindMemberElement(memberName));
    }
}
