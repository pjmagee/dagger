...
internal static class DaggerTracer
{
    ...
    {
        // Parent from the engine-provided TRACEPARENT when there is no live
        // span, so invocation spans join the CLI's trace instead of starting a
        // new root in a foreign trace.
        using var activity =
            System.Diagnostics.Activity.Current is null
            && TracePropagation.ParseActivityContext() is { } parentCtx
                ? DaggerTelemetryInitializer.ActivitySource.StartActivity(
                    name,
                    System.Diagnostics.ActivityKind.Internal,
                    parentCtx
                )
                : DaggerTelemetryInitializer.ActivitySource.StartActivity(name);
