...
public static class QueryExecutor
{
    ...
    )
    {
        ...
        foreach (
            var fieldName in path.RemoveAt(path.Count - 1)
                .Select(field => field.Name)
                // Inline fragment segments (`... on Type`, used for node-based
                // object loading) select into the parent object and produce no
                // nesting in the response JSON, so skip them when traversing.
                .Where(name => !name.StartsWith("... on ", StringComparison.Ordinal))
        )
        {
