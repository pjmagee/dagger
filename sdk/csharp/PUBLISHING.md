...
This guide explains how to build and publish the Dagger.SDK NuGet package.
...
- Maintainer access to the Dagger.SDK package
...
```bash
# From sdk/csharp directory
dagger call -m dev pack export --path=./packages

# This creates:
# packages/Dagger.SDK.0.1.0.nupkg
# packages/Dagger.SDK.0.1.0.snupkg (symbols)
```
...
# Output:
# Dagger.SDK.0.1.0.nupkg
# Dagger.SDK.0.1.0.snupkg
```
...
1. Visit https://www.nuget.org/packages/Dagger.SDK
  ...
  publish:
    ...
    steps:
      ...
      - name: Publish to NuGet
        run: |
          dotnet nuget push \
            sdk/csharp/src/bin/Release/Dagger.SDK.*.nupkg \
...
- [ ] Run all tests: `dagger call -m dev test`
- [ ] Build package: `dagger call -m dev pack export --path=./packages`
...
│   └── dotnet/
│       └── cs/
│           └── Dagger.SDK.CodeGen.dll (source generator)
├── README.md
└── [metadata]
```
...
dotnet nuget push packages/Dagger.SDK.0.1.0.nupkg \
    --skip-duplicate \
    --api-key $NUGET_API_KEY \
    --source https://api.nuget.org/v3/index.json
```
