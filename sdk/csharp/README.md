...
The SDK is managed with a Dagger module. From the root of the Dagger repository, you can use the `csharp-client` toolchain to work on the SDK:

```shell
dagger call -m csharp-client
```
...
```shell
dagger call -m csharp-client publish --version="0.1.0" --nuget-token=env:NUGET_TOKEN
```
